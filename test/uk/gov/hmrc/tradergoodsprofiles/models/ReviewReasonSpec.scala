  ...
  "ReviewReason" must {
    "throw a JsError" when {
      "deserializing from json and not a correct ReviewReason" in {
        Json.fromJson[ReviewReason](JsString("invalid")) mustBe JsError(
          "[ReviewReason] Reads unknown ReviewReason: invalid"
        )
      }

      "deserializing from json and not a JsString" in {
        Json.fromJson[ReviewReason](JsBoolean(true)) mustBe JsError(
          "[ReviewReason] Reads expected JsString but got: true"
        )
      }
    }
    ...
    "serialize to json" when {
      ...
      "Commodity" in {
        Json.toJson(Commodity: ReviewReason) mustBe JsString(
          "The commodity code has expired. You'll need to change the commodity code and categorise the goods."
        )
      }

      "Inadequate" in {
        Json.toJson(Inadequate: ReviewReason) mustBe JsString(
      ...
      "Unclear" in {
        Json.toJson(Unclear: ReviewReason) mustBe JsString(
      ...
      "Measure" in {
        Json.toJson(Measure: ReviewReason) mustBe JsString(
          "The commodity code or restrictions have changed. You'll need to categorise the record."
        )
      }

      "Mismatch" in {
        Json.toJson(Mismatch: ReviewReason) mustBe JsString(
